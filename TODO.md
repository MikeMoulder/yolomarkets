...
## Ops / hygiene
- [~] **web/.env.local `DEPLOYER_PRIVATE_KEY` is not the factory admin** (`0xcf03…` vs admin `0xdfB1…` in root `.env`) — server-side market creation from the Next app reverts `NotAdmin`. **Sidestepped 2026-08-01 for Telegram**: the command center now runs as the pm2 poller `yolo-telegram-bot`, which loads root `.env` first, so `/create` signs with the real admin key (verified: `/status` reports deployer == factory admin). Still open for anything that creates markets *from Vercel* — fix the key there (or in `web/.env.local` for local dev) before moving the bot back to the webhook transport.
